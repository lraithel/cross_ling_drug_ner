...
class Attribute(object):
    ...
    def equals(self, other, mode="strict"):
        ...
        assert mode in ("strict", "lenient")
        if (
            self.arg.equals(other.arg, mode)
            and self.arg.equals(other.arg, mode)
            and self.rtype == other.rtype
            and self.rval == other.rval
        ):
            return True
        return False
    ...
    def __str__(self):
        """String representation."""
        return "{} ({} dimension is {})".format(self.arg, self.rtype, self.rval)
...
class MultipleEvaluator(object):
    ...
    def __init__(self, corpora, tag_type=None, mode="strict", verbose=False):
        """Initialize."""
...
def evaluate(corpora, mode="strict", verbose=False):
    ...
    for rel in evaluator_s.attributes:
        ...
        evaluator_tag_s = MultipleEvaluator(
            corpora, rel, mode="strict", verbose=verbose
        )
        evaluator_tag_l = MultipleEvaluator(
            corpora, rel, mode="lenient", verbose=verbose
        )
        print(
            "{:>20}  {:<5.4f}  {:<5.4f}  {:<5.4f}    {:<5.4f}  {:<5.4f}  {:<5.4f}".format(
                "{}".format(rel),
    ...
    evaluator_tag_s = MultipleEvaluator(
        corpora, "Combined", mode="strict", verbose=verbose
    )
    evaluator_tag_l = MultipleEvaluator(
        corpora, "Combined", mode="lenient", verbose=verbose
    )
    print(
        "{:>20}  {:<5.4f}  {:<5.4f}  {:<5.4f}    {:<5.4f}  {:<5.4f}  {:<5.4f}".format(
            "{}".format(rel),
