...
from finetune_char_based import DrugNER
...
def convert_documents_to_brat(
    predictions, texts, txt_files, data_url, output_dir, test_data_identifier
):
    ...
    for prediction, file_name in zip(predictions, txt_files):

        path_to_text = os.path.join(data_url, test_data_identifier, file_name + ".txt")
...
if __name__ == "__main__":
    ...
    checkpoint_dirs = ["outputs/checkpoint_canine-s_23_01_11_10_55"]
    ...
    for no, checkpoint_dir in enumerate(checkpoint_dirs):
        ...
        combined_predictions, combined_texts, txt_files = utils.re_combine_windows(
            drug_ner.tokenized_datasets["test"], predictions
        )

        dir_wo_string_matching = "predicted_annotations/"

        output_dir = os.path.join(checkpoint_dir, dir_wo_string_matching)
