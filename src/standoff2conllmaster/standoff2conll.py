...
"""
Lisa Raithel: Added function "get_converted_files" (02/03/2022)
"""
...
def argparser():
    ...
    ap = argparse.ArgumentParser(
        description="Convert standoff to CoNLL format",
    ...
    ap.add_argument(
        "-a", "--asciify", default=None, action="store_true", help="map input to ASCII"
    )
    ap.add_argument(
        "-c",
        "--char-offsets",
    ...
    ap.add_argument(
        "-k",
        "--tokenization",
        choices=list(TOKENIZATION_REGEXS.keys()),
        default=list(TOKENIZATION_REGEXS.keys())[0],
        help="tokenization",
    )
    ap.add_argument(
        "-o",
        "--overlap-rule",
        choices=OVERLAP_RULES,
...
def convert_files(files, options):
    for fn in sorted(files):
        ...
        conll_data = document.to_conll(
            include_offsets=options.char_offsets, include_docid=options.include_docid
        )
        sys.stdout.write(conll_data)


def get_converted_files(files, options):
    data = []
    for fn in sorted(files):
        document = read_ann(fn, options)
        if options.singletype:
