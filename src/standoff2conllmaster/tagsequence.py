...
def main(argv=None):
    ...
    multi_args = [
        fix_bio_options.first_type,
        fix_bio_options.last_type,
        fix_bio_options.split_multi,
    ]
    assert (
        len([a for a in multi_args if a == True]) < 2
    ), 'At most one of the "-f", "-l" and "-s" arguments can be specified.'
    ...
    if fix_bio_options.indices is None:
        ...
        try:
            ...
            assert (
                False
            ), 'Argument "-i" value should be a comma-separated list of integers'
...
def argparser():
    ...
    ap.add_argument(
        ...
        default=False,
        action="store_true",
        help="Use first type in for BI+ sequences with multiple types.",
    )
    ap.add_argument(
        "-l",
        "--last-type",
        default=False,
        action="store_true",
        help="Use first type in for BI+ sequences with multiple types (default).",
    )
    ap.add_argument(
        "-s",
        "--split-multi",
        default=False,
        action="store_true",
        help="Split BI+ sequences with multiple types (add Bs).",
    )
    ap.add_argument(
        "-i",
        "--indices",
