...
class Textbound(object):
    ...
    def _parse_offsets(cls, offsets):
        ...
        if parsed != ordered:
            warn("using ordered spans {} instead of source {}".format(ordered, parsed))
            parsed = ordered
        return parsed
    ...
    def _resolve_discontinuous(cls, offsets, text, discont_rule=None):
        ...
        if discont_rule == FULL_SPAN:
            ...
            print(
                'Resolve discontinuous "%s" to last subspan "%s"' % (text, last_text),
                file=sys.stderr,
            )
    ...
    def from_str(cls, string, discont_rule=None):
        try:
            ...
            if len(offsets) != 1:
                offsets, text = cls._resolve_discontinuous(offsets, text, discont_rule)
                was_discontinuous = True
            start, end = offsets[0]
            ann = cls(id_, type_, start, end, text)
            ann.skip_validation = was_discontinuous and discont_rule != LAST_SPAN
            return ann
...
def eliminate_overlaps(textbounds, overlap_rule=None):
    ...
    for t1 in textbounds:
        for t2 in textbounds:
            ...
            try:
                print(
                    "Eliminate %s due to overlap with %s" % (elim, keep),
                    file=sys.stderr,
                )
            except UnicodeEncodeError:
                print(
                    "Eliminate %s due to overlap with %s" % (elim.id, keep.id),
                    file=sys.stderr,
                )
            eliminate[elim] = True
...
def verify_textbounds(textbounds, text, document_id):
    ...
    for t in textbounds:
        if t.skip_validation:
            ...
            print(
                'Resolve discontinuous "%s" to full span "%s"'
            ...
            try:
                ...
                s = "Doc ID %s:\nError verifying textbound %s: %s" % (document_id, t, e)
                # raise FormatError(s.encode("utf-8"))
                print(FormatError(s.encode("utf-8")))
                continue
        ...
        for o in range(token.start, token.end):
            if o in offset_ann:
                ...
                if o != token.start or tb.end < token.end:
                    ...
                    warn(
                        'annotation-token boundary mismatch: "%s" --- "%s"'
