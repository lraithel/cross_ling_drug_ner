...
class Token(object):
    ...
    def to_nersuite(self, exclude_tag=False):
        ...
        if len(self.text) > NERSUITE_TOKEN_MAX_LENGTH:
            ...
            print(
                "Warning: truncating very long token (%d characters) for NERsuite"
                % len(self.text),
                file=sys.stderr,
            )
...
class Sentence(object):
    ...
    def get_tagged(self, relative_offsets=False):
        ...
        if relative_offsets:
            tagged = [
                (t[0], t[1] - self.base_offset, t[2] - self.base_offset) for t in tagged
            ]
        return tagged
    ...
    def to_nersuite(self, exclude_tag=False):
        ...
        return "\n".join(chain((t.to_nersuite(exclude_tag) for t in tokens), ["\n"]))
    ...
    def standoffs(self, index):
        ...
        for type_, start, end in self.get_tagged():
            ...
            textbounds.append(
                Textbound("T%d" % index, type_, start, end, self.text[tstart:tend])
            )
            index += 1
        return textbounds
...
class Document(object):
        ...
        for s in self.sentences:
            ...
            for i, c in enumerate(s.text):
                if not c.isspace():
                    ...
                    o += 1
            tagged = [
                (t[0], offset_map[t[1]], offset_map[t[2] - 1], t[3]) for t in tagged
            ]

            for t in tagged:
                lines.append("%s|%d %d|%s\n" % (self.sentence_id(s), t[1], t[2], t[3]))

        return "".join(lines)

    def bc2gm_text(self):
        return "".join(
            ["%s %s\n" % (self.sentence_id(s), s.text) for s in self.sentences]
        )
    ...
    @classmethod
    def from_text(
        cls, text, sentence_split=True, annotations=None, tokenization_re=None
    ):
        ...
        for s in split:
            sentences.append(
                Sentence.from_text(s, offset, tokenization_re=tokenization_re)
            )
            offset += len(s)
    ...
    def from_standoff(
        ...
        document = cls.from_text(
            text, sentence_split, textbounds, tokenization_re=tokenization_re
        )
        ...
        if exclude_types:
            textbounds = filter_textbounds(textbounds, exclude_types, exclude=True)
        # print(f"\n\n Doc ID: {document.id}")
        verify_textbounds(textbounds, text, document_id)
