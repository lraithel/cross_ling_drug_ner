...
def read_mapping(f, fn="mapping data"):
    ...
    for i, l in enumerate(f):
        ...
        assert (
            c not in mapping or mapping[c] == r
...
def print_summary(out, mapping):
    ...
    for c in sk:
        try:
            print(
                "\t%.4X\t%s\t'%s'\t%d"
                % (ord(c), c.encode("utf-8"), mapping[c], map_count[c]),
                file=out,
            )
        except:
    ...
    for c in sk:
        try:
            print(
                "\t%.4X\t%s\t%d" % (ord(c), c.encode("utf-8"), missing_mapping[c]),
                file=out,
            )
        except:
...
def argparser():
    ...
    ap = argparse.ArgumentParser(
        description="Replaces Unicode characters in input text with ASCII approximations."
    )
    ap.add_argument(
        "-d",
        "--directory",
...
def log_missing_ascii_mappings(write=warn):
    if len(missing_mapping) == 0:
        return
    write("Characters without ASCII mapping: %d" % sum(missing_mapping.values()))
    sk = list(missing_mapping.keys())
    sk.sort(lambda a, b: cmp(missing_mapping[b], missing_mapping[a]))
    for c in sk:
        try:
            write("\t%.4X\t%s\t%d" % (ord(c), c.encode("utf-8"), missing_mapping[c]))
        except:
...
def main(argv):
    ...
    try:
        ...
        print(
            "Error reading mapping from %s: %s" % (MAPPING_FILE_NAME, e),
            file=sys.stderr,
        )
        return 1
